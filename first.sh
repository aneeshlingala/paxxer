...
if [[ "$ARCH" == "aarch64" ]]; then
    ...
    echo "Updating kernel from $(echo $KERNEL) to 6.4.7-stb-mt8"
    sudo rm -rf /boot/*
    sudo rm -rf /lib/modules/*
    cd ~
    sudo wget "https://github.com/hexdump0815/linux-mainline-mediatek-mt81xx-kernel/releases/download/6.4.7-stb-mt8%2B/6.4.7-stb-mt8+.tar.gz"
    cd /
    sudo tar -xzvf ~/6.4.7-stb-mt8+.tar.gz
    sudo rm -rf 6.4.7-stb-mt8+.tar.gz
    sudo dd if=/boot/vmlinux.kpart-6.4.7-stb-mt8+ of=/dev/mmcblk0p1 bs=1M status=progress
    sudo dd if=/boot/vmlinux.kpart-6.4.7-stb-mt8+ of=/dev/mmcblk0p2 bs=1M status=progress
fi
